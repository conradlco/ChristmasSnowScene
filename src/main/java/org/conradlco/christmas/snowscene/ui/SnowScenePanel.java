...
public class SnowScenePanel extends JPanel {
  ...
  private static final int NUMBER_OF_SNOWFLAKES = 200;
  ...
  private final List<Point> baubles = new ArrayList<>();
  ...
  public SnowFlake getRandomSnowFlake() {
    ...
    int depth = random.nextInt(SnowFlake.MAX_DEPTH);
    int fallSpeed = random.nextInt(2); // Random fall speed between 1 and 3
  ...
  private void paintTree(Graphics g) {
    ...
    try {
      ...
      // Compute total height of the tree (levels stacked + trunk) so we can align the base to bottom
  ...
  private void ensureBaublesInitialized(int w, int h) {
    ...
    Rectangle starRect = new Rectangle(centerX - starOuterRadius - 2, starCenterY - starOuterRadius - 2, starOuterRadius * 2 + 4, starOuterRadius * 2 + 4);
    ...
    for (int level = 0; level < levels; level++) {
      ...
      int baublesThisLevel = 3 + seedRand.nextInt(3);
      for (int b = 0; b < baublesThisLevel; b++) {
        ...
        Color[] palette = { Color.red, Color.cyan, Color.magenta, Color.orange, Color.yellow, new Color(0, 200, 0) };
